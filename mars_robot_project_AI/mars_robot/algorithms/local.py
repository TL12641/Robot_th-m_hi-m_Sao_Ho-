...
def _bfs_lay_duong_di_va_duyet(grid, diem_dau, diem_dich):
    """BFS tra ve (duong_di, tap_cac_o_da_duyet) de hien thi animation dung."""
    hang_doi = deque([diem_dau])
    den_tu   = {diem_dau: None}
    ...
    while hang_doi:
        ...
        if hien_tai == diem_dich:
            ...
            while o is not None:
                ...
                o = den_tu[o]
            duong_di.reverse()
            return duong_di, set(den_tu.keys())
...
def _xay_ket_qua(grid, vi_tri_dau, thu_tu_mau, vi_tri_base):
    ...
    tat_ca_da_xet = set()   # cac o BFS da mo trong qua trinh tim duong
    tong_buoc = 0
    tong_chi_phi = 0
    ...
    for diem_dich in cac_diem_can_den:
        duong_di, cac_o_duyet = _bfs_lay_duong_di_va_duyet(grid, vi_tri_hien_tai, diem_dich)
        ...
        tat_ca_da_xet.update(cac_o_duyet)
