...
def _chay_game(grid, start, samples, base, co_alpha_beta, do_sau):
    ...
    # Luu lich su gan day de phat hien lap
    # 12 buoc de bat duoc vong lap dai hon tren luoi 15x15
    LICH_SU_GAN_DAY = 12
    gan_day_a = []
    gan_day_b = []
    ...
    while cac_mau_con_lai and so_luot < GIOI_HAN_LUOT:
        ...
        # Phat hien lap: nuoc di tot nhat quay lai vi tri da di qua,
        # hoac vi tri hien tai xuat hien >= 2 lan trong lich su gan day
        bi_lap_a = (nuoc_di_tot_nhat_a in gan_day_a or
                    gan_day_a.count(vi_tri_a) >= 2)
        if bi_lap_a and cac_mau_con_lai:
        ...
        # Phat hien lap cho Robot B
        bi_lap_b = (nuoc_di_tot_nhat_b in gan_day_b or
                    gan_day_b.count(vi_tri_b) >= 2)
        if bi_lap_b and cac_mau_con_lai:
