    ...
    for muc_tieu in thu_tu:
        ...
        for i in range(len(duong_di) - 1):
            ...
            if grid[o_tiep_theo[0]][o_tiep_theo[1]] == ROUGH:
                ...
                # Xay ke hoach phuc hoi cho moi truong hop truot
                # Chi lay 1 truong hop de khong lam roi UI
                if o_co_the_truot:
                    o_truot = o_co_the_truot[0]
                    ...
                    if duong_phuc_hoi and len(duong_phuc_hoi) > 1:
                        ...
                        # print(f"[AND-OR] Plan B: truot tu {o_hien_tai} sang {o_truot}")
